...
dotchart <- function(x1, x2, x3, xlab, x3lab, labels, main, description, col="#789aa1") {
  ...
  xmax <- max(x2) * 1.05
  labels <- str_sub(labels, start=1, end=80)

  opar <- par(mai=c(.5,0,.75,0.5), omi=c(.5,.5,.5,.5), fin=c(10,7.5), lty=0)
  dotchart2(x1, labels=labels, auxdata=x3, auxtitle=x3lab, dotsize=1, lty=3, pch=1, xlim=c(0,xmax), reset.par=TRUE)
  dotchart2(x2, labels=labels, dotsize=1, pch=19, xlim=c(0,xmax), add=TRUE)
