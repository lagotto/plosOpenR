...
dotchart <- function(x1, x2, labels, xlab=NULL, x3="", x3lab=NULL, main=NULL, description=NULL, col="#789aa1") {
  ...
  cex.labels <- 0.75
  line_height <- 0.02
  lower_margin <- 0.8 - (length(x1) * line_height)

  opar <- par(mai=c(.5,0,.75,0.5), omi=c(.5,.5,.5,.5), fig=c(0,1,lower_margin,1), lty=0)
  ...
  mtext(xlab, side=1, col=col, cex=1, line=2, adj=1)
  par(opar)
}